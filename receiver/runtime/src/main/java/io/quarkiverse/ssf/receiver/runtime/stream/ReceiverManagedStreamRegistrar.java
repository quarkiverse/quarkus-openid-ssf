 ...
 *
 * <p>
 * When {@code quarkus.openid-ssf.receiver.stream-management=RECEIVER} and
 * {@code quarkus.openid-ssf.receiver.receiver-managed.register-stream=true} (the default), this
...
public class ReceiverManagedStreamRegistrar {
    ...
    void onStart(@Observes @Priority(200) StartupEvent event) {
        ...
        if (!config.receiverManaged().registerStream()) {
            LOG.infof(
                    "quarkus.openid-ssf.receiver.receiver-managed.register-stream=false — skipping startup stream registration");
            return;
        }
        ...
        URI deliveryUrl = push
                ? config.push().deliveryEndpointUrl().orElseThrow(() -> new IllegalStateException(
                        "quarkus.openid-ssf.receiver.push.delivery-endpoint-url is required when stream-management=RECEIVER and delivery-method=PUSH"))
                : null;
    ...
    private StreamConfiguration buildCreateRequest(URI deliveryUrl) {
        ...
        if (rawEntries.isEmpty()) {
            throw new IllegalStateException(
                    "quarkus.openid-ssf.receiver.events-requested must list at least one event URI or alias when stream-management=RECEIVER");
        }
