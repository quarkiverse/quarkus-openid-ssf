 ...
 */
package com.easyssf.quarkus.ssfreceiver.runtime.devui;
...
public class SsfDevJsonRpcService {
     ...
     *
     * <ul>
     * <li>{@code TRANSMITTER}: ready iff {@code ssf.receiver.stream-id} is configured.</li>
    ...
    public RegistrationStatus registrationStatus() {
        ...
        if (!enabled) {
            return new RegistrationStatus(false, config.streamManagement().name(), null,
                    "ssf.receiver.enabled=false — receiver disabled");
        }
        return switch (config.streamManagement()) {
            case TRANSMITTER -> {
                ...
                yield new RegistrationStatus(streamId != null, "TRANSMITTER", streamId,
                        streamId != null ? null : "ssf.receiver.stream-id is not configured");
            }
            case RECEIVER -> {
