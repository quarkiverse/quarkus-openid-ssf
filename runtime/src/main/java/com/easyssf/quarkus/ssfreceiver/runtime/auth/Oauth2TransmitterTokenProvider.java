 ...
 * from {@code ssf.receiver.oauth2.token-endpoint} with HTTP Basic /
...
public class Oauth2TransmitterTokenProvider implements TransmitterTokenProvider {
    ...
    void init() {
        ...
        this.tokenEndpoint = oauth2.tokenEndpoint().orElseThrow(() -> new IllegalStateException(
                "ssf.receiver.oauth2.token-endpoint is required when Oauth2TransmitterTokenProvider is active"));
        ...
        if (clientId == null) {
            throw new IllegalStateException(
                    "ssf.receiver.oauth2.client-id is required when ssf.receiver.oauth2.token-endpoint is set");
        }
        if (clientSecret == null) {
            throw new IllegalStateException(
                    "ssf.receiver.oauth2.client-secret is required when ssf.receiver.oauth2.token-endpoint is set");
        }
