# Receiver — design (as built)

Design notes for the **`quarkus-openid-ssf-receiver`** extension that lives
under `receiver/` in this repository. Sibling extensions (transmitter etc.)
will get their own design docs alongside this one.
...
| Outbound auth | ✅ | Static bearer / self-contained OAuth2 `client_credentials` / `quarkus-oidc-client` / Noop — selected at build time. |
| SET signature policy | ✅ | RS256-only by default with 2048-bit minimum RSA key (CAEP Interop §3.1); algorithm allowlist + minimum RSA key size are config-tunable via `set-validation.*`. |
...
| Disable kill-switch | ✅ | `quarkus.openid-ssf.receiver.enabled=false` no-ops every startup observer. |
...
|---|---|
| `receiver/runtime/` | Everything that runs in the consumer's JVM: config mapping, beans, REST clients, SPIs. Imports its optional integrations (`quarkus-oidc-client`, `quarkus-micrometer`) with `<optional>true</optional>` so consumers don't pay the transitive cost. |
| `receiver/deployment/` | The build-time `SsfReceiverProcessor` plus the Dev UI processor. Decides which token-provider / metrics impl to register based on classpath capabilities and `ConfigProvider`-readable config at build time. Hosts the smoke test. |
| `receiver/examples/example-transmitter-managed-stream/` | Consumer app — TRANSMITTER mode + PUSH against Keycloak. |
| `receiver/examples/example-receiver-managed-stream/` | Consumer app — RECEIVER mode against caep.dev (default profile, static-token + PUSH) or Keycloak (`-Dquarkus.profile=keycloak`, OIDC + POLL). |

The runtime artifact's `quarkus-extension.properties` descriptor points at
`io.quarkiverse.openid-ssf:quarkus-openid-ssf-receiver-deployment` so Quarkus
can locate the deployment processor at build time.

## Runtime components

```
receiver/runtime/src/main/java/io/quarkiverse/ssf/receiver/runtime/
├── SsfReceiverConfig.java                                # @ConfigMapping("quarkus.openid-ssf.receiver")
├── auth/
...
│   ├── StaticTransmitterTokenProvider.java              # registered when quarkus.openid-ssf.receiver.transmitter-access-token is set
│   ├── Oauth2TransmitterTokenProvider.java              # self-contained client_credentials grant (no quarkus-oidc-client dep)
...
| `stream-management` | enum | `RECEIVER` | `TRANSMITTER` or `RECEIVER`. |
...
| `events-requested` | List\<URI \| alias\> | unset | Required in RECEIVER mode when `register-stream=true` and no `stream-id` is pinned (sent as `events_requested` on create-stream). |
| `event-aliases.<alias>` | Map\<String, URI\> | built-ins for SSF / CAEP / RISC spec | Used as the `event` Micrometer tag. |
...
| `set-validation.accepted-algorithms` | List\<String\> | `RS256` | JWS `alg` allowlist enforced by `SetVerifier` (CAEP Interop §3.1). |
| `set-validation.min-rsa-key-size` | int | `2048` | Minimum RSA modulus size for `RS*` signatures. |
...
| `oauth2.token-endpoint` | URI | unset | Activates `Oauth2TransmitterTokenProvider` at build time (when no static token). |
| `oauth2.grant-type` | string | `client_credentials` | Token-endpoint grant. |
| `oauth2.client-id` / `client-secret` | string | unset | Client credentials. |
| `oauth2.client-auth-method` | enum | `BASIC` | `BASIC` (RFC 6749 §2.3.1) or `POST` (form body). |
| `oauth2.scopes` | List\<String\> | unset | Optional `scope` parameter. |
| `oauth2.additional-params` | Map\<String,String\> | empty | Extra form fields appended to the token request. |
| `oauth2.expiry-safety-window` | Duration | `30s` | Refresh-ahead window subtracted from `expires_in`. |
| `oauth2.timeout` | Duration | `5s` | HTTP timeout for the token call. |
| `oidc.token-timeout` | Duration | `2s` | Max wait when fetching a token from `OidcClient` (bounds the Vert.x event-loop block). |
...
`SsfReceiverProcessor` makes classpath- and config-dependent decisions at
build time. Each lives in its own `@BuildStep`; the resulting bean graph
is baked into the final jar / native image.

| Decision | Trigger | Effect |
|---|---|---|
| `StaticTransmitterTokenProvider` | `quarkus.openid-ssf.receiver.transmitter-access-token` is set (read via `ConfigProvider` at build time so the active profile is honored). | Wins outright; OAuth2 + OIDC steps short-circuit. |
| `Oauth2TransmitterTokenProvider` | `quarkus.openid-ssf.receiver.oauth2.token-endpoint` is set and no static token. | Self-contained `client_credentials` grant — no `quarkus-oidc-client` dependency required. Takes precedence over OIDC. |
| `OidcTransmitterTokenProvider` | `quarkus-oidc-client` is on the classpath (capability `quarkus.oidc-client`) and neither static nor OAuth2 is configured. | Fetches tokens via `OidcClient`. When none of the three above register, the `@DefaultBean` `NoopTransmitterTokenProvider` stays in effect and outbound calls go unauthenticated. |
| Micrometer metrics impl | `io.quarkus.metrics` capability (provided transitively by `quarkus-micrometer`). | When present, `MicrometerSsfReceiverMetrics` is registered (`@Startup`) so meters appear at `/q/metrics` from the first scrape, not on first event. Otherwise the noop default stays. |
| REST client interface indexing | always. | Every interface used by `RestClientBuilder.build(...)` is added to the Jandex index — required because the runtime jar isn't part of the application archive by default. |
...
Namespace `ssf.receiver.*` — intentionally distinct from the
`quarkus.openid-ssf.receiver.*` **config** prefix so existing consumer
dashboards / Prometheus alerts that reference the meter names are not a
hidden coupling to config schema. Pre-registered at startup so they appear
at the first scrape with value `0`:
...
- **Durable persistence defaults** — ship a `quarkus-openid-ssf-receiver-jdbc`
...
- **Transmitter extension** — a sibling `receiver/`-style module under
  the same `io.quarkiverse.openid-ssf` umbrella, providing the dual side
  of the wire so the same repo ships both ends of a stream.
